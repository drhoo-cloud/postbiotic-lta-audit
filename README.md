...
> Nguyen TTM, Kim H, Kim H-G, Chung DK, Yi T-H.
> *Lipoteichoic acid as a test case for what the genome cannot specify
> in postbiotics.* Submitted to npj Science of Food.
...
## Running it
    ...
    python scripts/05_aggregate.py             # writes Tables S3a-b
    python scripts/06_dispersion_test.py       # writes Table S4d
...
or one at a time:
    ...
    python figures/make_fig2_data.py          # regenerates fig2_data.json from Table S3d
    ...
    python figures/figS1_score_distribution.py        # Figure S1 (needs Table S3a)
...
`data/tables/TableS3d_per_species_localization.csv` and is regenerated, never edited by hand.
...
That DOI resolves to the most recent version. The release that accompanies the
submitted manuscript is v1.2.0, a complete re-run of the pipeline on 2026-09-23;
its own version DOI is shown on that release page.
